...
class DeckStats(QDialog):
    ...
    def __init__(self, mw):
        ...
        f.groups.connect(lambda: self.changeScope("deck"))
        f.groups.setShortcut("g")
        f.all.connect(lambda: self.changeScope("collection"))
        f.month.connect(lambda: self.changePeriod(0))
        f.year.connect(lambda: self.changePeriod(1))
        f.life.connect(lambda: self.changePeriod(2))
