...
class AddonManager(object):
    ...
    def rebuildAddonsMenu(self):
        ...
        for file in self.files():
            ...
            a.triggered.connect(lambda p=p: self.onEdit(p))
            m.addAction(a)
            a = QAction(_("Delete..."), self.mw)
            a.triggered.connect(lambda p=p: self.onRem(p))
            m.addAction(a)
    ...
    def onEdit(self, path):
        ...
        frm.buttonBox.connect(accepted, lambda: self.onAcceptEdit(path, frm))
        d.exec_()
